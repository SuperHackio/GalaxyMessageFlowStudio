    ...
    public class STNodeOption
    {
        ...
        public STNode Owner {
            ...
            internal set {
                ...
                if (_Owner != null) DisConnectionAll();        //当所有者变更时 断开当前所有连接
                _Owner = value;
            }
        }
        ...
        private readonly Color _SameOwnerColor = Color.Pink;
        private readonly Color _LoopColor = Color.Purple;
        private Color _LineCol = Color.Transparent;
        private Color _CurrentLineCol;
        /// <summary>
        /// 获取或设置当前 Option 连接点的颜色
        /// </summary>
        public Color LineColor
        {
            get { return _CurrentLineCol; }
            internal set
            {
                if (value == _DotColor) return;
                _LineCol = value;
                _CurrentLineCol = value;
                Invalidate();
            }
        }
        ...
        public Rectangle DotRectangle {
            get {
                return new Rectangle(_DotLeft, _DotTop, _DotSize, _DotSize);
            }
        }
        /// <summary>
        /// 获取当前 Option 被连接的个数
        ...
        protected internal virtual void OnConnected(STNodeOptionEventArgs e) {
            if (Connected != null/* && this._IsInput*/) Connected(this, e);
        }
        ...
        protected virtual bool ConnectingOption(STNodeOption op) {
            ...
            if (_Owner.Owner == null) return false;
        ...
        public virtual ConnectionStatus CanConnect(STNodeOption op) {
            _CurrentLineCol = _LineCol;
            if (this == STNodeOption.Empty || op == STNodeOption.Empty) return ConnectionStatus.EmptyOption;
            if (_IsInput == op.IsInput) return ConnectionStatus.SameInputOrOutput;
            if (op.Owner == null || _Owner == null) return ConnectionStatus.NoOwner;
            if (op.Owner == _Owner) _CurrentLineCol = _SameOwnerColor;// return ConnectionStatus.SameOwner;
            if (_Owner.LockOption || op._Owner.LockOption) return ConnectionStatus.Locked;
            if (_IsSingle && m_hs_connected.Count == 1) DisConnectionAll();
            Stack<STNode> temp = new();
            if (op.IsInput && STNodeEditor.CanFindNodePath(op.Owner, _Owner, ref temp)) _CurrentLineCol = _LoopColor;
            if (m_hs_connected.Contains(op)) return ConnectionStatus.Exists;
            if (_IsInput && op._DataType != _DataType && !op._DataType.IsSubclassOf(_DataType)) return ConnectionStatus.ErrorType;
        ...
        public virtual ConnectionStatus DisConnectOption(STNodeOption op) {
            ...
            if (_Owner == null) return ConnectionStatus.NoOwner;
        ...
        public List<STNodeOption> GetConnectedOption() {
            if (_DataType == null) return null;
            if (!_IsInput)
                return m_hs_connected.ToList();
            List<STNodeOption> lst = new();
            if (_Owner == null) return null;
            if (_Owner.Owner == null) return null;
        ...
        public void TransferData(object data, bool bDisposeOld) {
            if (bDisposeOld && _Data != null) {
                if (_Data is IDisposable) ((IDisposable)_Data).Dispose();
                _Data = null;
            }
            TransferData(data);
        }
        ...
        private bool AddConnection(STNodeOption op, bool bSponsor) {
            if (_DataType == null) return false;
            bool b = m_hs_connected.Add(op);
            OnConnected(new STNodeOptionEventArgs(bSponsor, op, ConnectionStatus.Connected));
            if (_IsInput) OnDataTransfer(new STNodeOptionEventArgs(bSponsor, op, ConnectionStatus.Connected));
            return b;
        }
        ...
        private bool RemoveConnection(STNodeOption op, bool bSponsor) {
            ...
            if (m_hs_connected.Contains(op)) {
                ...
                if (_IsInput) OnDataTransfer(new STNodeOptionEventArgs(bSponsor, op, ConnectionStatus.DisConnected));
